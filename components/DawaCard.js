...
const DawaCard = ({ item }) => {
  ...
  if (item.price.buyPrice == null && item.price.rentalPrice == null) {
    ...
    return (
      <View style={{ flex: 1, alignItems: 'center', backgroundColor: '#f8f8f8', padding: 20, borderRadius: '10%', marginTop: 5, marginBottom: 5, marginLeft: 5, marginRight: 5 }}>
        ...
        <View style={{ flexDirection: 'row', width: '100%' }}>
          <Text style={{ fontSize: 13, fontWeight: 'bold', color: 'green' }}>FREE</Text>
    ...
    const rentPrice = item.price.rentalPrice + '$' + '/' + item.price.rentalTimeframe + ' '


    return (
      <View style={{ backgroundColor: '#f8f8f8', padding: 20, borderRadius: '10%', marginTop: 5, marginBottom: 5, marginLeft: 5, marginRight: 5 }}>
        <Image style={{ alignSelf: 'center', width: '100%', height: 200, marginTop: 5, borderRadius: '10%', marginBottom: '5%' }} source={{ uri: item.itemImage }} />

        <View style={{ flexDirection: 'row' }}>
          <Text style={{ fontSize: 13, fontWeight: 'bold', color: '#b22222' }}> {buyPrefix}</Text>
          <Text style={{ fontSize: 13, fontWeight: 'bold', color: '#b22222' }}> {buyPrice}</Text>


          <Text style={{ fontSize: 13, fontWeight: 'bold', color: '#b22222' }}> {rentPrefix}</Text>
          <Text style={{ fontSize: 13, fontWeight: 'bold', color: '#b22222' }}> {rentPrice}</Text>
